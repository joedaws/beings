defmodule Cosmos.Locations.Node do
  alias Cosmos.Locations.Node

  @default_limit 2
  @data_path "./data"

  defstruct [
    :name,
    :type,
    occupancy: 0,
    occupancy_limit: @default_limit
  ]

  def get_name(node) do
    "#{node.name}"
  end

  def get_random_node_type() do
    path = Path.join(Path.expand(@data_path), "node_type_registry.yaml")

    {:ok, types} = YamlElixir.read_from_file(path)

    Enum.random(Map.get(types, "node_type"))
  end

  def generate_node(name, type \\ :no_name)

  def generate_node(name, type) when type != :no_name do
    %Node{
      name: name,
      type: type,
      occupancy: 0,
      occupancy_limit: @default_limit
    }
  end

  def generate_node(name, type) when type == :no_name do
    %Node{
      name: name,
      type: get_random_node_type(),
      occupancy: 0,
      occupancy_limit: @default_limit
    }
  end

  def generate_id(node) do
    :erlang.md5(get_name(node) <> (:os.system_time(:millisecond) |> to_string))
  end
end
