...
** Cosmos Application
*** TODO fix connect in cosmos.ex so that correct calls are used.
*** TODO fix events to string conversion so that we can record to file the changes.
*** TODO fix historian crash causing all other processes to crash.
This should be a restart setting somewhere
*** TODO use register node name, module name etc. IN node creation
