...
* Tasks
** TODO complete experimental genserver setup by running one life cycle.

** TODO replace some server_being callbacks with handle info

** TODO refactor beings and nodes to be genservers
...
* Spin up and Spin down tasks
Things to do while the stream is starting and stopping
** create weird eldrich beings names.
These are stored in the [[file:data/being_name_registry.yaml][name registry]] yaml.
** create node names
These are stored in the [[file:data/node_name_registry.yaml][node name registry]] yaml.
